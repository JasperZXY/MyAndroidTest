...
public class MyListView extends ListView implements OnScrollListener {
    ...
    //应用的时候可以看需要，对这个数据进行保存
    private long lastTimeRefresh = 0;
    ...
    public void stopRefresh() {
        if (mPullRefreshing == true) {
            ...
            resetHeaderHeight();
            lastTimeRefresh = System.currentTimeMillis();
            SimpleDateFormat format = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
            mHeaderTimeView.setText(format.format(new Date(lastTimeRefresh)));
        }
    }
...
//		mHeaderTimeView.setText(time);
        if (lastTimeRefresh > 0) {
            ...
            if (curTime - lastTimeRefresh < 60 * 1000) {  //不足一分钟
                mHeaderTimeView.setText("刚刚");
            } else if (curTime - lastTimeRefresh < 60 * 60 * 1000) {   //不足一小时
                mHeaderTimeView.setText((curTime - lastTimeRefresh) / 60000 + "分钟前");
            }
        }
    }
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
     ...
     * onMyScrolling when header/footer scroll back.
     */
